...
MFRC522::StatusCode MFRC522::PCD_CalculateCRC(byte *data, ///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
	...
	_driver->PCD_WriteRegister(PCD_Register::CommandReg,
			PCD_Command::PCD_CalcCRC);    // Start the calculation
	...
	for (uint16_t i = 5000; i > 0; i--) {
		...
		if (n & 0x04) {                  // CRCIRq bit set - calculation done
			_driver->PCD_WriteRegister(PCD_Register::CommandReg,
					PCD_Command::PCD_Idle); // Stop calculating CRC for new content in the FIFO.
...
//  }
	...
	_delay_ms(4); // Optional delay of 4ms. Some board do need more time after init to be ready, see Readme.
...
void MFRC522::PCD_Reset() {
	_driver->PCD_WriteRegister(PCD_Register::CommandReg,
			PCD_Command::PCD_SoftReset);  // Issue the SoftReset command.
...
MFRC522::StatusCode MFRC522::PCD_TransceiveData(byte *sendData, ///< Pointer to the data to transfer to the FIFO.
	...
	return PCD_CommunicateWithPICC(PCD_Command::PCD_Transceive, waitIRq,
...
MFRC522::StatusCode MFRC522::PCD_CommunicateWithPICC(byte command, ///< The command to execute. One of the PCD_Command enums.
	...
	if (backData && backLen) {
		...
		*backLen = n;                      // Number of bytes returned
		_driver->PCD_ReadRegister(PCD_Register::FIFODataReg, n, backData,
	...
	if (backData && backLen && checkCRC) {
		...
		MFRC522::StatusCode status = PCD_CalculateCRC(&backData[0],
...
MFRC522::StatusCode MFRC522::PICC_RequestA(byte *bufferATQA, ///< The buffer to store the ATQA (Answer to request) in
	...
	return PICC_REQA_or_WUPA(PICC_Command::PICC_CMD_REQA, bufferATQA,
			bufferSize);
} // End PICC_RequestA()
...
MFRC522::StatusCode MFRC522::PICC_WakeupA(byte *bufferATQA, ///< The buffer to store the ATQA (Answer to request) in
	...
	return PICC_REQA_or_WUPA(PICC_Command::PICC_CMD_WUPA, bufferATQA,
			bufferSize);
} // End PICC_WakeupA()
...
MFRC522::StatusCode MFRC522::PICC_REQA_or_WUPA(byte command, ///< The command to send - PICC_Command::PICC_CMD_REQA or PICC_Command::PICC_CMD_WUPA
	...
	status = PCD_TransceiveData(&command, 1, bufferATQA, bufferSize,
			&validBits);
	if (status != StatusCode::STATUS_OK) {
		return status;
	}
...
MFRC522::StatusCode MFRC522::PICC_Select(Uid *uid, ///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
	...
	while (!uidComplete) {
		...
		while (!selectDone) {
			...
			_driver->PCD_WriteRegister(PCD_Register::BitFramingReg,
			...
			if (result == StatusCode::STATUS_COLLISION) { // More than one PICC in the field => collision.
				byte valueOfCollReg = _driver->PCD_ReadRegister(
						PCD_Register::CollReg); // CollReg[7..0] bits are: ValuesAfterColl reserved CollPosNotValid CollPos[4:0]
...
MFRC522::StatusCode MFRC522::PCD_Authenticate(byte command, ///< PICC_Command::PICC_CMD_MF_AUTH_KEY_A or PICC_Command::PICC_CMD_MF_AUTH_KEY_B
	...
	return PCD_CommunicateWithPICC(PCD_Command::PCD_MFAuthent, waitIRq,
...
MFRC522::StatusCode MFRC522::MIFARE_Decrement(byte blockAddr, ///< The block (0-0xff) number.
	...
	return MIFARE_TwoStepHelper(PICC_Command::PICC_CMD_MF_DECREMENT, blockAddr,
			delta);
...
MFRC522::StatusCode MFRC522::MIFARE_Increment(byte blockAddr, ///< The block (0-0xff) number.
	...
	return MIFARE_TwoStepHelper(PICC_Command::PICC_CMD_MF_INCREMENT, blockAddr,
			delta);
...
MFRC522::StatusCode MFRC522::MIFARE_Restore(byte blockAddr ///< The block (0-0xff) number.
	...
	return MIFARE_TwoStepHelper(PICC_Command::PICC_CMD_MF_RESTORE, blockAddr,
			0L);
} // End MIFARE_Restore()
 ...
 * @return StatusCode::STATUS_OK on success, StatusCode::STATUS_??? otherwise.
 */
MFRC522::StatusCode MFRC522::PCD_NTAG216_AUTH(const byte password[4],
		byte pACK[]) {
...
//	byte cmdBufferSize	= sizeof(cmdBuffer);
	byte validBits = 0;
	byte rxlength = 5;
	result = PCD_CommunicateWithPICC(PCD_Command::PCD_Transceive, waitIRq,
...
void MFRC522::MIFARE_CalculateAccessBits(byte accessBitBuffer[3], ///< Pointer to byte 6, 7 and 8 in the sector trailer. Bytes [0..2] will be set.
	...
	const byte c1 = ((g3 & 4) << 1) | ((g2 & 4) << 0) | ((g1 & 4) >> 1)
			| ((g0 & 4) >> 2);
	const byte c2 = ((g3 & 2) << 2) | ((g2 & 2) << 1) | ((g1 & 2) << 0)
			| ((g0 & 2) >> 1);
	const byte c3 = ((g3 & 1) << 3) | ((g2 & 1) << 2) | ((g1 & 1) << 1)
			| ((g0 & 1) << 0);
